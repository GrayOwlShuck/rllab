...
def make_dense_layer(input_shape, num_units, name='fc', W=tf_layers.xavier_initializer(), b=tf.zeros_initializer(),
...
def forward_dense_layer(input, W, b, nonlinearity=tf.identity, batch_norm=False, scope='', reuse=True,
                        is_training=False):
    ...
    if batch_norm:
        return tf_layers.batch_norm(activation, activation_fn=nonlinearity, reuse=reuse, scope=scope,
...
class SensitiveLfdGaussianMLPPolicy(StochasticPolicy, Serializable):
    def __init__(
            self,
            name,
            env_spec,
        ...
        if std_network is not None:
            ...
            super(SensitiveLfdGaussianMLPPolicy, self).__init__(env_spec)
    ...
    def compute_updated_lfd_dists(self, samples, step=0):
        """ 
        Do N gradient steps (Compute fast gradients once) and pull params out of tensorflow for sampling.
        :param samples: list of demos on which to run BC
        """
        ...
        obs_demo_list, action_demo_list = [], []
        for i in range(num_tasks):
            inputs = ext.extract(samples[i],
                                 'observations', 'actions')
            obs_demo_list.append(inputs[0])
            action_demo_list.append(inputs[1])

        inputs = obs_demo_list + action_demo_list  # each is a list, where entries correspond to diff envs/tasks

        # To do a second update, replace self.all_params below with the params that were used to collect the policy.
        init_param_values = None
        if self.all_param_vals is not None:  # if it's not before the first update
        ...
        if 'all_fast_params_tensor' not in dir(self):
            ...
            for i in range(num_tasks):
                gradients = dict(zip(update_param_keys, tf.gradients(self.surr_objs[i], [self.all_params[key] for key in
                                                                                         update_param_keys])))
                self.grad_norms.append(tf.norm(tf.concat([tf.reshape(g, [-1]) for g in gradients.values()], 0)))
                fast_params_tensor = dict(zip(update_param_keys,
        ...
        avg_grad_norms = sess.run(tf.reduce_mean(self.grad_norms), feed_dict=dict(list(zip(self.input_list_for_grad, inputs))))
        logger.log("The avg gradient size across tasks is: " + str(avg_grad_norms))
        logger.record_tabular('update%i_AvgGradNorm' % step, avg_grad_norms)
        surr_objs = sess.run(self.surr_objs, feed_dict=dict(list(zip(self.input_list_for_grad, inputs))))
        avg_surr_obj = np.mean(surr_objs)
        # avg_surr_obj = sess.run(tf.reduce_mean(self.surr_objs), feed_dict=dict(list(zip(self.input_list_for_grad, inputs))))
        logger.record_tabular('update%ipre_AvgLfDLoss' % step, avg_surr_obj)
        logger.log("The pre-update avg surr_obj are: " + str(surr_objs))
        # todo: how to compute the loss for the post-update?
        ...
        ##CF: here just repeating the assignment to be able to compute the post-update surrogate
        for i in range(num_tasks):
            if self.all_param_vals is not None:  # if it's not before the first update
                self.assign_params(self.all_params, self.all_param_vals[i])
        # avg_surr_obj = sess.run(tf.reduce_mean(self.surr_objs), feed_dict=dict(list(zip(self.input_list_for_grad, inputs))))
        surr_objs = sess.run(self.surr_objs, feed_dict=dict(list(zip(self.input_list_for_grad, inputs))))
        avg_surr_obj = np.mean(surr_objs)
        logger.record_tabular('update%ipost_AvgLfDLoss' % step, avg_surr_obj)
        logger.log("The post-update avg surr_obj are: " + str(surr_objs))

        if init_param_values is not None:  # todo: why resetting the values of the params to the pre-update ones?
    ...
    def updated_dist_info_sym(self, task_id, surr_obj, new_obs_var, params_dict=None, is_training=True):
        ...
        gradients = dict(
            zip(update_param_keys, tf.gradients(surr_obj, [old_params_dict[key] for key in update_param_keys])))
        params_dict = dict(
            zip(update_param_keys, [old_params_dict[key] - step_size * gradients[key] for key in update_param_keys]))
    ...
    def updated_dist_info_lfd_sym(self, task_id, surr_obj, new_obs_demo_vars, params_dict=None, is_training=True):  # todo: why new_obs_demo is not used?
        """ symbolically create MAML graph, for the meta-optimization, only called at the beginning of meta-training.
        Called more than once if you want to do more than one grad step.
        """
        old_params_dict = params_dict
        ...
        return self.dist_info_sym(new_obs_demo_vars, all_params=params_dict, is_training=is_training)  # CF? why give the dist info of these obs
                                                                                                       # instead of just the new params?

    @overrides
    def get_action(self, observation, idx=0):
    ...
    def get_actions(self, observations, idx=None):
        ...
        # if idx is not none, all the obs are collected from the same policy, related to that env.
    ...
    def create_MLP(self, name, output_dim, hidden_sizes,
        ...
        with tf.variable_scope(name):
            if self.mask_units == True:
                all_params['mask_units'] = add_param(tf.zeros_initializer(), (cur_shape[1],), layer_name='mask_units',
                                                     name='0', regularizable=False)
