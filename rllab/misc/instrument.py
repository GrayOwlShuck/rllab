...
def launch_ec2(params_list, exp_prefix, docker_image, code_full_path,
    ...
    for params in params_list:
        ...
        if periodic_sync:
            exclude_all = " --exclude '*' " if not sync_all_data_node_to_s3 else " "
            ...
            sio.write("""
                while /bin/true; do
                    aws s3 sync {exclude_all} {include_png} {include_pkl} {include_log} {include_html}--include '*.csv' --include '*.json' {log_dir} {remote_log_dir}
                    sleep {periodic_sync_interval}
                done & echo sync initiated""".format(exclude_all=exclude_all,
                                                     include_png=include_png, include_pkl=include_pkl,
